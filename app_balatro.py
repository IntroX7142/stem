...
# ====================== TẢI DỮ LIỆU THIÊN THỂ ======================
try:
    planets = load('de421.bsp')
except Exception as e:
    st.error("Không thể tải tệp dữ liệu de421.bsp. Vui lòng kiểm tra kết nối internet lần đầu chạy.")
    st.stop()
    
earth = planets['earth']

# ====================== CƠ SỞ DỮ LIỆU THIÊN THỂ ======================
db_thien_the = {
    # Hệ Mặt Trời
...
# ====================== HÀM TÍNH TOÁN CHUNG ======================
def compute_alt_az(obj):
    """Tính Alt/Az cho bất kỳ thiên thể nào"""
    ts = load.timescale()
    t = ts.now()
    nguoi_quan_sat = earth + THCS_MINH_DUC
    astrometric = nguoi_quan_sat.at(t).observe(obj)
    alt, az, _ = astrometric.apparent().altaz()
    return alt.degrees, az.degrees

def tinh_toan_vi_tri(thien_the_chon):
    """Tính vị trí + khoảng cách"""
    doi_tuong = db_thien_the[thien_the_chon]
    alt, az = compute_alt_az(doi_tuong)
    
    # Khoảng cách chỉ có cho hành tinh
    if thien_the_chon in ["☀️ MẶT TRỜI", "🌕 MẶT TRĂNG"] or "Sao" in thien_the_chon[:8]:
        # Tính distance cho hành tinh
        ts = load.timescale()
        t = ts.now()
        nguoi_quan_sat = earth + THCS_MINH_DUC
        astrometric = nguoi_quan_sat.at(t).observe(doi_tuong)
        _, _, distance = astrometric.apparent().altaz()
        dist_str = f"{distance.km:,.0f} km" if hasattr(distance, 'km') else "Xa vô tận"
    else:
        dist_str = "Xa vô tận (hàng trăm năm ánh sáng)"
    
    return alt, az, dist_str

def ve_ban_do_sao_tuong_tac():
    """Vẽ bản đồ sao tương tác bằng Plotly (tất cả thiên thể)"""
    ts = load.timescale()
    t = ts.now()
    nguoi_quan_sat = earth + THCS_MINH_DUC
...
with col_btn:
    if st.button("📍 BẮT ĐẦU DÒ TÌM & QUAN SÁT", use_container_width=True):
        ...
        if alt > 0:
            st.success(f"**✅ CÓ THỂ QUAN SÁT NGAY!**  \nQuay kính hướng **{az:.0f}°** (Bắc = 0°), nâng góc **{alt:.0f}°**. {ten_ngan} đang ở trên bầu trời Minh Đức!")
        else:
            st.error(f"**❌ DƯỚI CHÂN TRỜI**  \n{ten_ngan} chưa xuất hiện. Hãy chờ thêm {int(-alt)} phút nữa!")
        
        # FUN FACT CARD
        st.markdown(f"""
        <div class="fun-fact">
            <h4 style="margin:0 0 8px 0; color:#67e8f9;">🌟 FUN FACT</h4>
            <p style="margin:0; font-size:1.25rem; line-height:1.5;">{fun_facts[lua_chon]}</p>
        </div>
        """, unsafe_allow_html=True)
...
st.caption("Di chuột vào các điểm để xem thông tin chi tiết & fun fact • Xanh = Có thể quan sát")
fig = ve_ban_do_sao_tuong_tac()
